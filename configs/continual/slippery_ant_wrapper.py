...
DEFAULT_SLIPPERY_FRICTIONS_CSV = Path(__file__).with_name("frictions.csv")


@lru_cache(maxsize=None)
def load_seeded_friction_schedule_table(csv_path: str) -> np.ndarray:
    """Loads the seeded friction table from CSV.

    Expected format:
    - optional header row of phase indices: 0,1,2,...
    - each subsequent row corresponds to one seed
    - each column corresponds to one post-default friction phase
    """
    path = Path(csv_path)
    if not path.exists():
        raise FileNotFoundError(f"Missing slippery friction CSV: {path}")

    with path.open(newline="") as handle:
        rows = [row for row in csv.reader(handle) if row]

    if not rows:
        raise ValueError(f"Slippery friction CSV is empty: {path}")

    num_cols = len(rows[0])
    header = np.asarray(rows[0], dtype=np.float32)
    has_header = np.allclose(
        header,
        np.arange(num_cols, dtype=np.float32),
        atol=0.0,
        rtol=0.0,
    )
    data_rows = rows[1:] if has_header else rows
    if not data_rows:
        raise ValueError(f"Slippery friction CSV has no data rows: {path}")

    for row_index, row in enumerate(data_rows):
        if len(row) != num_cols:
            raise ValueError(
                f"Slippery friction CSV row {row_index} has {len(row)} columns; "
                f"expected {num_cols}."
            )

    table = np.asarray(data_rows, dtype=np.float32)
    if table.ndim != 2 or table.shape[1] == 0:
        raise ValueError(f"Invalid slippery friction CSV shape {table.shape}: {path}")
    return table
...
class AutoResetTimeStepWrapper(AutoResetWrapper):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        state.info['timestep'] += self.action_repeat
        return state
...
class NonstationaryFrictionBraxWrapper(BraxGymnaxWrapper):
    ...
    def __init__(self, env_name, backend="positional",
        ...
        if self.schedule_seed < 0 or self.schedule_seed >= friction_table.shape[0]:
            raise ValueError(
                f"schedule_seed={self.schedule_seed} is out of range for "
                f"{friction_table.shape[0]} seeded friction rows."
            )
        self.friction_schedule = jnp.asarray(
            friction_table[self.schedule_seed], dtype=jnp.float32
        )
        self.num_phases = int(self.friction_schedule.shape[0])

        env = envs.get_environment(env_name=env_name, backend=backend)
        self.max_steps_in_episode = 1000
        env = EpisodeWrapper(env, episode_length=self.max_steps_in_episode, action_repeat=action_repeat)
    ...
    def step(self, rng: chex.PRNGKey, state: State, action: jnp.ndarray, params=None):
        ...
        # Keep the default friction for the first phase, then traverse the
        # seeded friction CSV row and hold the last value if the run is longer.
        phase = state.info['timestep'] // self.change_every
        friction = self._phase_friction(phase)
