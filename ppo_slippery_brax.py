#!/usr/bin/env python
"""PPO harness for testing the SlipperyAnt Brax wrapper."""
import os
import random
import sys
import time
...
from configs.continual.slippery_ant_wrapper import (  # noqa: E402
...
class Args:
    ...
    # Action repeat
    action_repeat: int = 1
    """Number of physics steps per policy action"""

    # SlipperyAnt wrapper behavior
    slippery_change_every: int = 100_000
    """Per-env steps between friction changes"""
    slippery_schedule_seed: Optional[int] = None
    """Seed selecting the row in configs/continual/frictions.csv; defaults to the training seed"""
...
def make_brax_envs(args):
    """Create vectorized SlipperyAnt wrapper environments."""
...
if __name__ == "__main__":
    ...
    key, reset_key = jax.random.split(key)
    reset_rngs = jax.random.split(reset_key, args.n_envs)
    next_obs, env_state = envs.reset(reset_rngs, None)
    ...
    def step_once(carry, step):
        ...
        next_obs, env_state, raw_reward, next_done, info = envs.step(
            step_rngs,
            env_state,
            action,
            None,
        )
        next_done = next_done.astype(jnp.bool_)  # Ensure bool type
        ...
        rollout_info = SlipperyRolloutInfo(
            ...
            timestep=env_state.info["timestep"],
        )
        return (
            agent_state,
            episode_stats,
    ...
    for iteration in range(1, args.num_updates + 1):
        ...
        if iteration % args.log_interval == 0:
            ...
            slippery_metrics = latest_slippery_metrics(rollout_info, args)
