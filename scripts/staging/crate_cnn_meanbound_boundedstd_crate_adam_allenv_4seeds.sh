#!/bin/bash
#SBATCH --job-name=stg-cratecnn-boundstd-adam-allenv4
#SBATCH --output=slurm_logs/staging_crate_cnn_meanbound_boundedstd_crate_adam_allenv_4seeds_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Adam-only staging ablation for the successful mean-bound +
# learnable bounded-logstd CRATECNN run.
# Same architecture/config as:
#   scripts/staging/crate_cnn_meanbound_boundedstd_crate_muon_allenv_4seeds.sh
# but with actor/critic heads routed to Adam instead of manifold MUON.
#
# Swept:
