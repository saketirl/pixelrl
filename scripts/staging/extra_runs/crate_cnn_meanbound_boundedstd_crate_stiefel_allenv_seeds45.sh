#!/bin/bash
#SBATCH --job-name=stg2-cratecnn-boundstd-allenv2
#SBATCH --output=slurm_logs/staging2_crate_cnn_meanbound_boundedstd_crate_stiefel_allenv_seeds45_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Extra staging seeds for the successful mean-bound + learnable bounded-logstd
# CRATECNN run. Same config as the 4-seed staging run, but with seeds 4 and 5.
#
# Swept:
...
GROUP_NAME="staging2_crate_cnn_meanbound_boundedstd_crate_stiefel_allenv_seeds45_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="staging,staging2,crate_cnn_meanbound_boundedstd,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},encoder_crate_cnn,actor_mean_tanh,actor_mean_scale_${ACTOR_MEAN_SCALE},bounded_global_logstd,actor_logstd_init_${ACTOR_LOGSTD_INIT},actor_logstd_min_${ACTOR_LOGSTD_MIN},actor_logstd_max_${ACTOR_LOGSTD_MAX},opt_stiefel,headarch_crate,sigreg_off,encoder_crate_step_${ENCODER_CRATE_STEP_SIZE},head_crate_step_${CRATE_STEP_SIZE},allenv9,seeds45"

EXP_NAME="ppo_staging2_cratecnn_meanbound_boundedstd_crate_stiefel_${ENV_NAME}_b${BACKEND}_s${SEED}_t${TASK_ID}"

echo "Running TASK_ID=${TASK_ID}/${NUM_CONFIGS} group=${GROUP_NAME}"
echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=crate_cnn encoder_crate_step_size=${ENCODER_CRATE_STEP_SIZE} actor_mean_tanh=true actor_mean_scale=${ACTOR_MEAN_SCALE} bounded_global_logstd=true actor_logstd_init=${ACTOR_LOGSTD_INIT} actor_logstd_min=${ACTOR_LOGSTD_MIN} actor_logstd_max=${ACTOR_LOGSTD_MAX} crate_head=true heads_optimizer=stiefel sigreg=off seed=${SEED}"
echo "Exp: ${EXP_NAME}"
