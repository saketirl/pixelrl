#!/bin/bash
#SBATCH --job-name=best-innovation-ant-humanoid-4seeds
#SBATCH --output=slurm_logs/best_innovation_ant_humanoid_4seeds_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
#   Muon heads on
#   CRATE heads on
#   innovation on with best current humanoid hyperparameters
...
SEED_IDX=$((TASK_ID / NUM_ENVS))

ENV_NAME="${ENVS[$ENV_IDX]}"
SEED="${SEEDS[$SEED_IDX]}"

GROUP_ID="${SLURM_ARRAY_JOB_ID:-${SLURM_JOB_ID:-local}}"
GROUP_NAME="best_innovation_ant_humanoid_4seeds_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="best_innovation,innovation_aux,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},headopt_muon,headarch_crate,innovation_on,proj_64,seed_sweep"

EXP_NAME="ppo_muon_${ENV_NAME}_bestinnovation_crate_innovationon_s${SEED}_t${TASK_ID}"
