#!/bin/bash
#SBATCH --job-name=innovation-aux-plain-heads
#SBATCH --output=slurm_logs/innovation_aux_plain_heads_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Keeps the same innovation sweep as the CRATE-head run, but with plain heads.
# Sweep:
...
GROUP_NAME="innovation_aux_plain_heads_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="innovation_aux,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},proj_${PROJ_DIM},innovation_coef_${INNOVATION_COEF},headopt_muon,headarch_plain"

EXP_NAME="ppo_muon_${ENV_NAME}_innovationaux_plain_proj${PROJ_DIM}_coef${COEF_IDX}_s${SEED}_t${TASK_ID}"
