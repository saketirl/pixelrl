#!/bin/bash
#SBATCH --job-name=projector-only-crate-heads
#SBATCH --output=slurm_logs/projector_only_crate_heads_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Compare these directly against the existing muon + crate-heads baseline.
...
GROUP_NAME="projector_only_crate_heads_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="projector_only,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},opt_muon,proj_${PROJ_DIM},crate_heads_on,encoder_crate_${CRATE_CONDITION},sigreg_off"

EXP_NAME="ppo_muon_${ENV_NAME}_crateheads_projectoronly_proj${PROJ_DIM}_enccrate${CRATE_CONDITION}_s${SEED}_t${TASK_ID}"
