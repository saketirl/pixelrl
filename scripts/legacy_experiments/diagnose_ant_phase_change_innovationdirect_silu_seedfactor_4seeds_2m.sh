#!/bin/bash
#SBATCH --job-name=diagnose-ant-phase-change-innovationdirect-silu-seedfactor-4seeds-2m
#SBATCH --output=slurm_logs/diagnose_ant_phase_change_innovationdirect_silu_seedfactor_4seeds_2m_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
GROUP_NAME="diagnose_ant_phase_change_innovationdirect_silu_seedfactor_${FACTOR_TAG}_fix${FIXED_SEED}_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="ant_phase_change,diagnosis,env_ant,backend_${BACKEND},seedfactor,${FACTOR_TAG},fixed_seed_${FIXED_SEED},varied_seed_${VARIED_SEED},init_seed_${INIT_SEED},data_seed_${DATA_SEED},headopt_muon,headarch_crate,encoder_innovation_direct,encoder_hiddenact_silu,encoder_final_muon,innovation_on,innovation_projdim_${INNOVATION_PROJ_DIM},innovation_warmup_${INNOVATION_WARMUP_UPDATES},innovation_ramp_${INNOVATION_RAMP_UPDATES},lr_anneal_off,probe_off,2m_steps"

EXP_NAME="ppo_muon_ant_phasechange_diag_innovationdirect_silu_seedfactor_${FACTOR_TAG}_fix${FIXED_SEED}_i${INIT_SEED}_d${DATA_SEED}_p${INNOVATION_PROJ_DIM}_w${INNOVATION_WARMUP_UPDATES}_r${INNOVATION_RAMP_UPDATES}_t${TASK_ID}"

echo "Running TASK_ID=${TASK_ID}/${#SEEDS[@]} group=${GROUP_NAME}"
echo "Config: env=ant backend=${BACKEND} factor_mode=${FACTOR_MODE} fixed_seed=${FIXED_SEED} varied_seed=${VARIED_SEED} init_seed=${INIT_SEED} data_seed=${DATA_SEED} encoder=innovation_direct_cnn hidden_activation=silu crate_head=true anneal_lr=false encoder_final_muon=true innovation_on=true innovation_proj_dim=${INNOVATION_PROJ_DIM} innovation_warmup_updates=${INNOVATION_WARMUP_UPDATES} innovation_ramp_updates=${INNOVATION_RAMP_UPDATES} encoder_muon_lr=${ENCODER_MUON_LR} encoder_muon_max_grad_norm=${ENCODER_MUON_MAX_GRAD_NORM} probe_interval=0"
echo "Exp: ${EXP_NAME}"
...
COMMON_ARGS=(
  ...
  --encoder-hidden-activation silu
  --sigreg-mode off
