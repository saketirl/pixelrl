#!/bin/bash
#SBATCH --job-name=maze-cratecnn-crateheads-s0
#SBATCH --output=slurm_logs/maze_crate_cnn_meanbound_boundedstd_crate_heads_ant_umaze_seed0_stiefel_adam_%j.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# One-GPU, one-seed maze launcher adapted from:
#   scripts/staging/crate_cnn_meanbound_boundedstd_crate_stiefel_allenv_4seeds.sh
#   scripts/staging/crate_cnn_meanbound_boundedstd_crate_adam_allenv_4seeds.sh
#
# Runs the same CRATE-CNN + CRATE-head config twice concurrently on the same
# GPU allocation:
...
run_variant() {
  ...
  local exp_name="ppo_maze_cratecnn_meanbound_boundedstd_crate_${heads_optimizer}_${ENV_NAME}_b${BACKEND}_s${SEED}"

  export WANDB_TAGS="${COMMON_TAGS},opt_${heads_optimizer}"

  echo "Running ${heads_optimizer} variant in group=${GROUP_NAME}"
  echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=crate_cnn crate_head=true heads_optimizer=${heads_optimizer} seed=${SEED} wandb_project=${WANDB_PROJECT} jax_mem_fraction=${JAX_MEM_FRACTION}"
  echo "Exp: ${exp_name}"
