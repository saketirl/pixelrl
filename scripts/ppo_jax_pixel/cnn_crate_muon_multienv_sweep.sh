#!/bin/bash
#SBATCH --job-name=cnn-crate-muon-multi
#SBATCH --output=slurm_logs/cnn_crate_muon_multi_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# on walker2d, cheetah, reacher, ant environments, 4 seeds.
#
# Swept:
#   env                 in {walker2d, cheetah, reacher, ant}
...
ENVS=(walker2d cheetah reacher ant)
BACKENDS=(spring spring generalized spring)
...
GROUP_NAME="cnn_crate_muon_multi_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="cnn_crate_muon,encoder_cnn,opt_muon,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},scale_${ENCODER_SCALE},crate,sweep4seeds"

EXP_NAME="ppo_cnn_crate_muon_scale${ENCODER_SCALE}_${ENV_NAME}_b${BACKEND}_s${SEED}_t${TASK_ID}"

echo "Running TASK_ID=${TASK_ID}/${NUM_CONFIGS} group=${GROUP_NAME}"
echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=cnn encoder_scale=${ENCODER_SCALE} crate_step_size=0.1 optimizer=muon seed=${SEED}"
echo "Exp: ${EXP_NAME}"
