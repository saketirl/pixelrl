#!/bin/bash
#SBATCH --job-name=cnn-vit-muon-gn
#SBATCH --output=slurm_logs/cnn_vit_heads_muon_gn_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Compare encoder architecture (CNN vs ViT) with MUON optimizer
...
NUM_SEEDS=${#SEEDS[@]}
NUM_CONFIGS=$((NUM_ENVS * NUM_ENCODERS * NUM_GRAD_NORMS * NUM_SEEDS))
...
ENC_IDX=$(((TASK_ID / (NUM_SEEDS * NUM_GRAD_NORMS)) % NUM_ENCODERS))
ENV_IDX=$((TASK_ID / (NUM_SEEDS * NUM_GRAD_NORMS * NUM_ENCODERS)))
...
GROUP_NAME="cnn_vit_heads_muon_gn_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="cnn_vit_heads_muon_gn,encoder_${ENCODER_TYPE},opt_muon,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},agn_${ACTOR_GRAD_NORM},cgn_${CRITIC_GRAD_NORM},sweep4seeds"

EXP_NAME="ppo_cmp_${ENCODER_TYPE}_muon_${ENV_NAME}_agn${ACTOR_GRAD_NORM}_cgn${CRITIC_GRAD_NORM}_b${BACKEND}_s${SEED}_t${TASK_ID}"

echo "Running TASK_ID=${TASK_ID}/${NUM_CONFIGS} group=${GROUP_NAME}"
echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=${ENCODER_TYPE} opt=muon actor_gn=${ACTOR_GRAD_NORM} critic_gn=${CRITIC_GRAD_NORM} seed=${SEED}"
echo "Exp: ${EXP_NAME}"
