#!/bin/bash
#SBATCH --job-name=cnn-crate-muon-ha45
#SBATCH --output=slurm_logs/cnn_crate_muon_ha45_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
GROUP_NAME="cnn_crate_muon_ha45_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="cnn_crate_muon,encoder_cnn,opt_muon,env_${ENV_NAME},backend_${BACKEND},seed_${SEED},scale_${ENCODER_SCALE},crate,sweep_seeds45"
...
echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=cnn encoder_scale=${ENCODER_SCALE} crate_step_size=${CRATE_STEP_SIZE} optimizer=muon seed=${SEED}"
echo "Exp: ${EXP_NAME}"
