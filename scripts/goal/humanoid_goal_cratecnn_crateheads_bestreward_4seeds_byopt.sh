#!/bin/bash
#SBATCH --job-name=hum-goal-cratecnn
#SBATCH --output=slurm_logs/humanoid_goal_cratecnn_crateheads_bestreward_4seeds_byopt_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Focused humanoid_goal run using the current best ant reward structure:
...
GROUP_NAME="humanoid_goal_cratecnn_crateheads_${CONFIG_NAME}_${HEADS_OPTIMIZER}_4seeds_ar${ACTION_REPEAT}_${GROUP_ID}"
...
run_seed() {
  local seed="$1"
  local exp_name="ppo_goal_reward_${CONFIG_NAME}_cratecnn_crateheads_${HEADS_OPTIMIZER}_ar${ACTION_REPEAT}_humanoid_goal_b${BACKEND}_s${seed}_long"
  local tags="goal,pixel_goal,humanoid_goal,reward_focus,${CONFIG_NAME},progress_${PROGRESS_SCALE},distance_${DISTANCE_SCALE},success_${SUCCESS_REWARD},action_repeat_${ACTION_REPEAT},env_humanoid_goal,backend_${BACKEND},seed_${seed},encoder_crate_cnn,headarch_crate,opt_${HEADS_OPTIMIZER},one_optimizer_per_gpu,mem_fraction_${JAX_MEM_FRACTION},gif_every_1m"
...
echo "Focused humanoid config=${CONFIG_NAME} optimizer=${HEADS_OPTIMIZER}"
