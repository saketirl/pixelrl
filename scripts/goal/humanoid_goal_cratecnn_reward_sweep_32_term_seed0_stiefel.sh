#!/bin/bash
#SBATCH --job-name=hum-goal-rs32-term
#SBATCH --output=slurm_logs/humanoid_goal_cratecnn_reward_sweep_32_term_seed0_stiefel_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Follow-up humanoid reward sweep for the fixed 2.5m, 45-degree goal.
# This sweep terminates unhealthy humanoids to avoid fallen/below-floor rollouts.
#
# 32 configs total:
...
#
# Each Slurm array task launches 4 configs on one GPU.
...
PROGRESS_SCALE="20"
HEALTHY_REWARD="5"
SUCCESS_EASY_REWARD="10"
SUCCESS_REWARD="50"

STANDING_SCALES=(0 2 4 8)
HEADING_SCALES=(0.5 1)
DISTANCE_SCALES=(0.1 0.25 0.5 0.75)

NUM_STANDING=${#STANDING_SCALES[@]}
NUM_HEADING=${#HEADING_SCALES[@]}
NUM_DISTANCE=${#DISTANCE_SCALES[@]}
NUM_CONFIGS=$((NUM_STANDING * NUM_HEADING * NUM_DISTANCE))
CONFIGS_PER_TASK=4
NUM_TASKS=8
...
GROUP_NAME="humanoid_reward_sweep32_term_cratecnn_stiefel_seed0_goal2p5_${GROUP_ID}"
...
run_config() {
  local config_id="$1"
  local distance_idx=$((config_id % NUM_DISTANCE))
  local heading_idx=$(((config_id / NUM_DISTANCE) % NUM_HEADING))
  local standing_idx=$((config_id / (NUM_DISTANCE * NUM_HEADING)))
  ...
  local config_name="stand${standing_tag}_heading${heading_tag}_dist${distance_tag}"
  local gif_dir="${SWEEP_DIR}/${config_name}"
  local exp_name="ppo_humanoid_goal_rs32term_${config_name}_cratecnn_stiefel_s${SEED}_goal2p5"
  local tags="goal,pixel_goal,humanoid_goal,reward_sweep32_term,goal_dist_2p5,terminate_unhealthy,encoder_crate_cnn,headarch_crate,opt_stiefel,seed_${SEED},action_repeat_${ACTION_REPEAT},gif_every_1m,progress_${PROGRESS_SCALE},healthy_${HEALTHY_REWARD},success_easy_${SUCCESS_EASY_REWARD},success_${SUCCESS_REWARD},gated_progress,stand_${standing_tag},heading_${heading_tag},distance_${distance_tag}"

  mkdir -p "${gif_dir}"

  echo "Running config_id=${config_id}/${NUM_CONFIGS} ${config_name}"
  echo "standing=${standing_scale} heading=${heading_scale} distance=${distance_scale}"
  echo "Exp: ${exp_name}"
...
echo "Humanoid terminating reward sweep task=${TASK_ID} group=${GROUP_NAME}"
