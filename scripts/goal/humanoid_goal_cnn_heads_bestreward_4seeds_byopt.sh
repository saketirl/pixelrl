#!/bin/bash
#SBATCH --job-name=hum-goal-cnn
#SBATCH --output=slurm_logs/humanoid_goal_cnn_heads_bestreward_4seeds_byopt_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Focused humanoid_goal run with the baseline full-run CNN encoder/heads and current best ant reward structure:
...
GROUP_NAME="humanoid_goal_cnn_heads_${CONFIG_NAME}_${HEADS_OPTIMIZER}_4seeds_ar${ACTION_REPEAT}_${GROUP_ID}"
...
run_seed() {
  local seed="$1"
  local exp_name="ppo_goal_reward_${CONFIG_NAME}_cnn_heads_${HEADS_OPTIMIZER}_ar${ACTION_REPEAT}_humanoid_goal_b${BACKEND}_s${seed}_long"
  local tags="goal,pixel_goal,humanoid_goal,reward_focus,${CONFIG_NAME},progress_${PROGRESS_SCALE},distance_${DISTANCE_SCALE},success_${SUCCESS_REWARD},action_repeat_${ACTION_REPEAT},env_humanoid_goal,backend_${BACKEND},seed_${seed},encoder_cnn,encoder_scale_${ENCODER_SCALE},headarch_mlp,opt_${HEADS_OPTIMIZER},one_optimizer_per_gpu,mem_fraction_${JAX_MEM_FRACTION},gif_every_1m"
...
echo "Focused humanoid CNN config=${CONFIG_NAME} optimizer=${HEADS_OPTIMIZER}"
