#!/bin/bash
#SBATCH --job-name=hum-goal-exp16
#SBATCH --output=slurm_logs/humanoid_goal_exp_distance_sweep_16_seed0_stiefel_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Humanoid goal exponential distance-potential sweep for the fixed 2.5m,
# 45-degree goal. Config 0 is the previous best close-reward control.
#
...
GROUP_NAME="humanoid_exp_distance_sweep16_cratecnn_stiefel_seed0_goal2p5_${GROUP_ID}"
SWEEP_DIR="${REPO_ROOT}/outputs/goal_exp_distance_sweep/${GROUP_NAME}"
mkdir -p "${SWEEP_DIR}"
...
config_values() {
  local config_id="$1"
  case "${config_id}" in
    0) echo "control_close_r0p75_c25_s250_stand0 250 0 25 0.75 0 1.0" ;;
    1) echo "exp_s25_t0p5 250 0 0 1.0 25 0.5" ;;
    2) echo "exp_s25_t1p0 250 0 0 1.0 25 1.0" ;;
    3) echo "exp_s25_t1p5 250 0 0 1.0 25 1.5" ;;
    4) echo "exp_s50_t0p5 250 0 0 1.0 50 0.5" ;;
    5) echo "exp_s50_t1p0 250 0 0 1.0 50 1.0" ;;
    6) echo "exp_s50_t1p5 250 0 0 1.0 50 1.5" ;;
    7) echo "exp_s50_t2p0 250 0 0 1.0 50 2.0" ;;
    8) echo "exp_s100_t0p5 250 0 0 1.0 100 0.5" ;;
    9) echo "exp_s100_t1p0 250 0 0 1.0 100 1.0" ;;
    10) echo "exp_s100_t1p5 250 0 0 1.0 100 1.5" ;;
    11) echo "exp_s100_t2p0 250 0 0 1.0 100 2.0" ;;
    12) echo "exp_s200_t0p5 250 0 0 1.0 200 0.5" ;;
    13) echo "exp_s200_t1p0 250 0 0 1.0 200 1.0" ;;
    14) echo "exp_s200_t1p5 250 0 0 1.0 200 1.5" ;;
    15) echo "exp_s200_t2p0 250 0 0 1.0 200 2.0" ;;
...
run_config() {
  ...
  local close_radius
  local exp_scale
  local exp_temperature
  read -r config_name success_reward standing_scale close_scale close_radius exp_scale exp_temperature < <(config_values "${config_id}")
  ...
  local exp_name="ppo_humanoid_goal_exp16_${config_name}_cratecnn_stiefel_s${SEED}_goal2p5"
  local tags="goal,pixel_goal,humanoid_goal,humanoid_exp_distance_sweep16,goal_dist_2p5,terminate_unhealthy,encoder_crate_cnn,headarch_crate,opt_stiefel,seed_${SEED},action_repeat_${ACTION_REPEAT},gif_every_1m,final_gif,healthy_${HEALTHY_REWARD},progress_${PROGRESS_SCALE},success_easy_${SUCCESS_EASY_REWARD},distance_${DISTANCE_SCALE},heading_${HEADING_SCALE},success_${success_tag},stand_${standing_tag},close_scale_${close_scale_tag},close_radius_${close_radius_tag},exp_distance_scale_${exp_scale_tag},exp_distance_temp_${exp_temperature_tag}"
  if [[ "${config_id}" == "0" ]]; then
    tags="${tags},baseline_control,previous_best_close"
  fi

  mkdir -p "${gif_dir}"

  echo "Running config_id=${config_id}/${NUM_CONFIGS} ${config_name}"
  echo "success=${success_reward} standing=${standing_scale} close_scale=${close_scale} close_radius=${close_radius} exp_scale=${exp_scale} exp_temperature=${exp_temperature}"
  echo "Exp: ${exp_name}"
...
echo "Humanoid exp distance sweep task=${TASK_ID} group=${GROUP_NAME}"
