#!/bin/bash
#SBATCH --job-name=cl-walker2d-10tasks
#SBATCH --output=slurm_logs/cl_walker2d_10tasks_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Continual Walker2d launcher using the same PPO/MUON hyperparameters as
# scripts/full_run.sh, restricted to Walker2d spring dynamics. This runs one
# seed for each head optimizer condition: MUON heads on and MUON heads off.
# Each run sees ten tasks total by default: task 0 uses default Brax dynamics,
# then nine sampled tasks.
...
GROUP_NAME="cl_walker2d_${NUM_TASKS}tasks_heads_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="continual_rl,continual_dynamics,walker2d,backend_${BACKEND},seed_${SEED},tasks_${NUM_TASKS},switch_${SWITCH_EVERY_ENV_STEPS},heads_${HEADS_CONDITION},cnn_heads_muon_me_hparams"

EXP_NAME="ppo_cl_walker2d_${NUM_TASKS}tasks_heads_${HEADS_CONDITION}_s${SEED}_t${TASK_ID}"

BASE_CONFIG="${REPO_ROOT}/configs/continual/walker2d_dynamics.yaml"
RUN_CONFIG="${REPO_ROOT}/slurm_logs/continual_walker2d_${NUM_TASKS}tasks_${GROUP_ID}_${HEADS_CONDITION}_${TASK_ID}.yaml"
