#!/bin/bash
#SBATCH --job-name=px-slip-single
#SBATCH --output=slurm_logs/pixelbrax_slippery_single_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Single-run PixelBrax SlipperyAnt calibration.
# Goal: maximize per-run progress under the 8h wall-clock limit to see how many
# friction phases PixelBrax can traverse without GPU packing.
...
RUN_NAMES=(plain_adam_lr1e4 plain_stiefel_lr1e4)
HEADS_OPTIMIZERS=(adam stiefel)
...
GROUP_NAME="cl_pixelbrax_slippery_ant_single_calibration_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export XLA_PYTHON_CLIENT_MEM_FRACTION="${JAX_MEM_FRACTION}"
export PYTHONUNBUFFERED=1
export WANDB_TAGS="continual_rl,pixelbrax,slippery_ant,single_gpu_calibration,config_${RUN_NAME},plainheads,headopt_${HEADS_OPTIMIZER},env_${ENV_NAME},backend_${BACKEND},seed_${SEED},lr_${LR},phase_every_${PHASE_EVERY_ENV_STEPS},change_every_per_env_${SLIPPERY_CHANGE_EVERY},phases_${EFFECTIVE_PHASES},action_repeat_${ACTION_REPEAT},crate_cnn,batch_${ROLLOUT_ENV_STEPS},minibatches_${NUM_MINIBATCHES},epochs_${UPDATE_EPOCHS},minibatch_size_${MINIBATCH_SIZE},jax_mem_fraction_${JAX_MEM_FRACTION},runs_per_gpu_1"

EXP_NAME="ppo_pixelbrax_slippery_ant_${RUN_NAME}_single_s${SEED}"

echo "PixelBrax SlipperyAnt single-run calibration group=${GROUP_NAME}"
