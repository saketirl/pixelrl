#!/bin/bash
#SBATCH --job-name=px-slip-sense
#SBATCH --output=slurm_logs/pixelbrax_slippery_sense_check_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# PixelBrax SlipperyAnt sense check based on slippery_ant_single_calibration.sh.
# Keeps the best calibration settings and enables task-boundary spectrum logging.
...
GROUP_NAME="cl_pixelbrax_slippery_ant_sense_check_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export XLA_PYTHON_CLIENT_MEM_FRACTION="${JAX_MEM_FRACTION}"
export PYTHONUNBUFFERED=1
export WANDB_TAGS="continual_rl,pixelbrax,slippery_ant,sense_check,spectrum_logging,config_${RUN_NAME},plainheads,headopt_${HEADS_OPTIMIZER},env_${ENV_NAME},backend_${BACKEND},seed_${SEED},lr_${LR},phase_every_${PHASE_EVERY_ENV_STEPS},change_every_per_env_${SLIPPERY_CHANGE_EVERY},phases_${EFFECTIVE_PHASES},action_repeat_${ACTION_REPEAT},crate_cnn,batch_${ROLLOUT_ENV_STEPS},minibatches_${NUM_MINIBATCHES},epochs_${UPDATE_EPOCHS},minibatch_size_${MINIBATCH_SIZE},jax_mem_fraction_${JAX_MEM_FRACTION},runs_per_gpu_1"

EXP_NAME="ppo_pixelbrax_slippery_ant_${RUN_NAME}_sense_check_s${SEED}"
