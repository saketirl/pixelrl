...
GROUP_NAME="cl_brax_inverted_pendulum_${NUM_TASKS}tasks_${ACTOR_CRITIC_ACTIVATION}_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="continual_rl,continual_dynamics,brax_state,inverted_pendulum,backend_${BACKEND},seed_${SEED},tasks_${NUM_TASKS},switch_${SWITCH_EVERY_ENV_STEPS},actorcritic_${ACTOR_CRITIC_ACTIVATION},constant_lr,state_ppo_debug"

EXP_NAME="ppo_brax_cl_inverted_pendulum_${NUM_TASKS}tasks_${ACTOR_CRITIC_ACTIVATION}_constantlr_s${SEED}"
...
echo "Config: env=${ENV_NAME} backend=${BACKEND} seed=${SEED} tasks=${NUM_TASKS} switch_every_env_steps=${SWITCH_EVERY_ENV_STEPS} total_timesteps=${TOTAL_TIMESTEPS} actor_critic_activation=${ACTOR_CRITIC_ACTIVATION}"
