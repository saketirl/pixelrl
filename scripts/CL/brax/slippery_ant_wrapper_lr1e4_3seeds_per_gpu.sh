#!/bin/bash
#SBATCH --job-name=slippery-ant-3seeds
#SBATCH --output=slurm_logs/slippery_ant_wrapper_3seeds_%j.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Runs multiple independent default SlipperyAnt PPO seeds on one allocated GPU.
# Per-run PPO defaults match slippery_ant_wrapper_lr1e4.sh; only the JAX memory
# fraction and process-level concurrency differ.
...
ACTOR_CRITIC_ACTIVATION="${6:-${ACTOR_CRITIC_ACTIVATION:-relu}}"
BACKEND="${7:-${BACKEND:-spring}}"
HEADS_OPTIMIZER="${8:-${HEADS_OPTIMIZER:-adam}}"
...
JAX_MEM_FRACTION="${JAX_MEM_FRACTION:-0.25}"
SEEDS="${SEEDS:-0 1 2}"
SLIPPERY_SCHEDULE_SEED_OVERRIDE="${SLIPPERY_SCHEDULE_SEED:-}"
...
SEEDS_PER_GPU="${#SEED_LIST[@]}"
GROUP_NAME="slippery_ant_wrapper_${BACKEND}_${DISTINCT_PHASES}phases_${ACTOR_CRITIC_ACTIVATION}_headopt_${HEADS_OPTIMIZER}_${GROUP_ID}_lr1e4_${SEEDS_PER_GPU}seeds_per_gpu"

echo "Running ${SEEDS_PER_GPU} SlipperyAnt seeds per GPU group=${GROUP_NAME}"
echo "Config: env=${ENV_NAME} backend=${BACKEND} seeds=${SEEDS} action_repeat=${ACTION_REPEAT} phase_every_env_steps=${PHASE_EVERY_ENV_STEPS} change_every_per_env_policy_steps=${CHANGE_EVERY_POLICY_STEPS} change_every_wrapper_steps=${CHANGE_EVERY} schedule=${SCHEDULE_DESC} total_timesteps=${TOTAL_TIMESTEPS} per_env_steps=${EFFECTIVE_ENV_STEPS_PER_ENV}"
echo "PPO per child: batch=${ROLLOUT_ENV_STEPS} lr=1e-4 epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} minibatch_size=${MINIBATCH_SIZE} clip_eps=0.1 reward_normalize=true clip_vloss=true max_grad_norm=0.05"
echo "JAX: XLA_PYTHON_CLIENT_MEM_FRACTION=${JAX_MEM_FRACTION}"
echo "W&B: entity=${WANDB_ENTITY} project=${WANDB_PROJECT}"
echo "Child seeds: ${SEEDS}"
...
for SEED in "${SEED_LIST[@]}"; do
  if [[ -n "${SLIPPERY_SCHEDULE_SEED_OVERRIDE}" ]]; then
    SLIPPERY_SCHEDULE_SEED="${SLIPPERY_SCHEDULE_SEED_OVERRIDE}"
  else
    SLIPPERY_SCHEDULE_SEED="${SEED}"
  fi
  EXP_NAME="ppo_slippery_brax_${ENV_NAME}_${BACKEND}_${DISTINCT_PHASES}phases_${ACTOR_CRITIC_ACTIVATION}_headopt_${HEADS_OPTIMIZER}_s${SEED}_lr1e4_${SEEDS_PER_GPU}seeds_per_gpu"
  CHILD_LOG="${REPO_ROOT}/slurm_logs/slippery_ant_wrapper_3seeds_${GROUP_ID}_seed${SEED}.out"
  LOGS+=("${CHILD_LOG}")
