#!/bin/bash
#SBATCH --job-name=ant-rlopt-lop-orig
#SBATCH --output=slurm_logs/ant_rlopt_lop_original_config_seed0_%A_%a.out
#SBATCH --error=slurm_logs/ant_rlopt_lop_original_config_seed0_%A_%a.err
#SBATCH -N 1
#SBATCH --ntasks=1
...
if (( TASK_ID < 0 || TASK_ID > 1 )); then
  echo "Invalid TASK_ID=${TASK_ID}. Expected 0 for Adam or 1 for Stiefel." >&2
  exit 1
fi
