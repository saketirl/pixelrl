...
#SBATCH --output=slurm_logs/ant_rlopt_lop_plain_adam_seeds1_5_%A_%a.out
#SBATCH --error=slurm_logs/ant_rlopt_lop_plain_adam_seeds1_5_%A_%a.err
#SBATCH -N 1
#SBATCH --ntasks=1
...
if (( TASK_ID < 1 || TASK_ID > 5 )); then
  echo "Invalid TASK_ID=${TASK_ID}. Expected 1..5 for seed-completion array." >&2
  exit 1
fi

CONDITION="rlopt_lop_plain_adam"
SEED="${TASK_ID}"
...
export WANDB_TAGS="continual_rl,slippery_ant,${CONDITION},plain_adam,seed_${SEED},backend_${BACKEND},network_arch_${NETWORK_ARCH},actorcritic_${ACTOR_CRITIC_ACTIVATION},no_reward_norm,no_obs_norm,unbounded_global_logstd,no_actor_mean_tanh,pixelrl_ppo_objective,serial_array"
