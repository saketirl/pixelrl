#!/bin/bash
#SBATCH --job-name=slip-hum-success
#SBATCH --output=slurm_logs/slippery_humanoid_success_%j.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Successful Slippery Humanoid state-observation PPO config.
# This is the Adam baseline that learned default-friction Humanoid and showed
# collapse under many rapid slippery-friction tasks.
...
GROUP_NAME="slippery_humanoid_success_adam_crate_obsnorm_${GROUP_ID}"
EXP_NAME="ppo_brax_sliphum_success_adam_crate_obsnorm_ph${NUM_PHASES}_phase${PHASE_EVERY_ENV_STEPS}_sched${SLIPPERY_SCHEDULE_SEED}_s${SEED}"

export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="continual_rl,slippery_humanoid_success,brax_state,humanoid,backend_${BACKEND},seed_${SEED},schedule_seed_${SLIPPERY_SCHEDULE_SEED},phase_every_${PHASE_EVERY_ENV_STEPS},phases_${NUM_PHASES},action_repeat_${ACTION_REPEAT},adam,lr_${LEARNING_RATE},obs_norm,crate_network,crate_heads,bounded_std,actor_mean_tanh"

echo "Running successful Slippery Humanoid config group=${GROUP_NAME}"
...
echo "PPO: lr=${LEARNING_RATE} anneal_lr=true batch=${ROLLOUT_ENV_STEPS} minibatch_size=${MINIBATCH_SIZE} epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES}"
