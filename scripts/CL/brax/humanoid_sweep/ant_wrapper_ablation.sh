#!/bin/bash
#SBATCH --job-name=ant-wrap-abl
#SBATCH --output=slurm_logs/slippery_ant_wrapper_ablation_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Ant-only 4 vs 4 ablation using the cadence from
# scripts/CL/brax/slippery_ant_wrapper.sh, with PPO/model hyperparameters
# matched to the successful Humanoid sweep recipe:
# - tasks 0..3: Stiefel heads, seeds 0..3
# - tasks 4..7: Adam heads, seeds 0..3
...
PHASE_EVERY_ENV_STEPS="${PHASE_EVERY_ENV_STEPS:-4999936}"
...
GROUP_NAME="slippery_ant_wrapper_ablation_${BACKEND}_${EFFECTIVE_PHASES}phases_${ACTOR_CRITIC_ACTIVATION}_${GROUP_ID}"
RUN_KIND="${HEADS_OPTIMIZER}_heads"
EXP_NAME="ppo_slippery_ant_wrapper_ablation_${BACKEND}_${EFFECTIVE_PHASES}phases_${ACTOR_CRITIC_ACTIVATION}_${RUN_KIND}_s${SEED}"

export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="continual_rl,slippery_ant_wrapper_ablation,slippery_ant_wrapper,humanoid_ppo_recipe,brax_state,${ENV_NAME},backend_${BACKEND},seed_${SEED},phase_every_${PHASE_EVERY_ENV_STEPS},change_every_per_env_${CHANGE_EVERY},phases_${EFFECTIVE_PHASES},requested_phases_${NUM_PHASES},action_repeat_${ACTION_REPEAT},actorcritic_${ACTOR_CRITIC_ACTIVATION},baseopt_${BASE_OPTIMIZER},weight_decay_${WEIGHT_DECAY},headopt_${HEADS_OPTIMIZER},wrapper_faithful,default_then_csv_schedule,batch_${ROLLOUT_ENV_STEPS},minibatches_${NUM_MINIBATCHES},epochs_${UPDATE_EPOCHS},minibatch_size_$((ROLLOUT_ENV_STEPS / NUM_MINIBATCHES)),reward_norm,vclip,obs_norm,crate_network,crate_head,bounded_global_logstd,actor_mean_tanh,lr_${LEARNING_RATE},repo_ppo"

echo "Running SlipperyAnt wrapper ablation group=${GROUP_NAME}"
