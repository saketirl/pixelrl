#!/bin/bash
#SBATCH --job-name=ant-lop-adam-6seed
#SBATCH --output=slurm_logs/slippery_ant_lop_plain_adam_%A_%a.out
#SBATCH --error=slurm_logs/slippery_ant_lop_plain_adam_%A_%a.err
#SBATCH -N 1
#SBATCH --ntasks=1
...
if [[ ! -f "${REPO_ROOT}/ppo_slippery_ant_loss_of_plasticity_adam.py" ]]; then
...
if (( TASK_ID < 0 || TASK_ID >= 6 )); then
  echo "Invalid TASK_ID=${TASK_ID}. Expected 0..5." >&2
  exit 1
fi

CONDITION="${CONDITION:-lop_plain_adam_clip_actions}"
...
WANDB_GROUP="${WANDB_GROUP:-slippery-ant-lop-plain-adam-6seeds}"
...
COMMAND=(
  uv run python "${REPO_ROOT}/ppo_slippery_ant_loss_of_plasticity_adam.py"
  --env-name "${ENV_NAME}"
  --backend "${BACKEND}"
  --n-envs "${N_ENVS}"
  --track
