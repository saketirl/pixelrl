#!/bin/bash
#SBATCH --job-name=ant-adam-cad
#SBATCH --output=slurm_logs/slippery_ant_adam_cadence_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Fast Adam-only Ant cadence sweep. This keeps the successful Humanoid PPO
# recipe, uses the Ant slippery wrapper path, and compresses the 20 scheduled
# friction phases into progressively shorter total budgets. Set
# CADENCE_PROFILE=aggressive for sub-125k phase lengths.
...
case "${CADENCE_PROFILE}" in
  standard)
    case "${cadence_idx}" in
      0) RUN_KIND="20phase_0p125m"; PHASE_EVERY_ENV_STEPS=124928 ;;
      1) RUN_KIND="20phase_0p25m"; PHASE_EVERY_ENV_STEPS=249984 ;;
      2) RUN_KIND="20phase_0p5m"; PHASE_EVERY_ENV_STEPS=499968 ;;
      3) RUN_KIND="20phase_1m"; PHASE_EVERY_ENV_STEPS=999936 ;;
      *) echo "Invalid cadence index ${cadence_idx}." >&2; exit 1 ;;
    esac
    ;;
  aggressive)
    case "${cadence_idx}" in
      0) RUN_KIND="20phase_0p031m"; PHASE_EVERY_ENV_STEPS=31232 ;;
      1) RUN_KIND="20phase_0p062m"; PHASE_EVERY_ENV_STEPS=62464 ;;
      2) RUN_KIND="20phase_0p125m"; PHASE_EVERY_ENV_STEPS=124928 ;;
      3) RUN_KIND="20phase_0p25m"; PHASE_EVERY_ENV_STEPS=249984 ;;
      *) echo "Invalid cadence index ${cadence_idx}." >&2; exit 1 ;;
    esac
    ;;
  *)
    echo "Unsupported CADENCE_PROFILE=${CADENCE_PROFILE}. Expected standard or aggressive." >&2
    exit 1
    ;;
esac
...
LR_TAG="${LR_TAG//-/_}"
GROUP_NAME="slippery_ant_adam_cadence_${CADENCE_PROFILE}_lr${LR_TAG}_${BACKEND}_${GROUP_ID}"
EXP_NAME="ppo_slippery_ant_adam_cadence_${RUN_KIND}_${BACKEND}_s${SEED}"

export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="continual_rl,slippery_ant_adam_cadence,slippery_ant_wrapper,humanoid_ppo_recipe,brax_state,${ENV_NAME},backend_${BACKEND},seed_${SEED},schedule_seed_${SLIPPERY_SCHEDULE_SEED},cadence_${CADENCE_PROFILE},${RUN_KIND},phase_every_${PHASE_EVERY_ENV_STEPS},change_every_per_env_${CHANGE_EVERY},phases_${EFFECTIVE_PHASES},requested_phases_${NUM_PHASES},action_repeat_${ACTION_REPEAT},actorcritic_${ACTOR_CRITIC_ACTIVATION},baseopt_${BASE_OPTIMIZER},weight_decay_${WEIGHT_DECAY},headopt_${HEADS_OPTIMIZER},default_then_csv_schedule,batch_${ROLLOUT_ENV_STEPS},minibatches_${NUM_MINIBATCHES},epochs_${UPDATE_EPOCHS},reward_norm,vclip,obs_norm,crate_network,crate_head,bounded_global_logstd,actor_mean_tanh,lr_${LEARNING_RATE},repo_ppo"

echo "Running SlipperyAnt Adam cadence sweep group=${GROUP_NAME}"
echo "Config: task_id=${TASK_ID} run_kind=${RUN_KIND} env=${ENV_NAME} backend=${BACKEND} seed=${SEED} action_repeat=${ACTION_REPEAT} phase_every_env_steps=${PHASE_EVERY_ENV_STEPS} change_every_policy_steps=${CHANGE_EVERY_POLICY_STEPS} change_every_wrapper_steps=${CHANGE_EVERY} schedule=default_then_csv schedule_seed=${SLIPPERY_SCHEDULE_SEED} total_timesteps=${TOTAL_TIMESTEPS} per_env_steps=${EFFECTIVE_ENV_STEPS_PER_ENV}"
echo "PPO: batch=${ROLLOUT_ENV_STEPS} lr=${LEARNING_RATE} base_optimizer=${BASE_OPTIMIZER} weight_decay=${WEIGHT_DECAY} adam_eps=${ADAM_EPS} anneal_lr=true epochs=${UPDATE_EPOCHS} minibatches=${NUM_MINIBATCHES} clip_eps=0.1 reward_normalize=true clip_vloss=true max_grad_norm=0.05"
