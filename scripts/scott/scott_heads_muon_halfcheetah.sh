#!/bin/bash
#SBATCH --job-name=scott-hc-heads
#SBATCH --output=slurm_logs/scott_halfcheetah_heads_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# Run two SCOTT PPO jobs on halfcheetah with identical PPO hyperparameters:
...
GROUP_NAME="scott_halfcheetah_heads_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="scott,halfcheetah,opt_${OPT_CONDITION},backend_${BACKEND},seed_${SEED}"

EXP_NAME="ppo_scott_${OPT_CONDITION}_${ENV_NAME}_b${BACKEND}_s${SEED}_t${TASK_ID}"

echo "Running TASK_ID=${TASK_ID}/${NUM_CONFIGS} group=${GROUP_NAME}"
echo "Config: env=${ENV_NAME} backend=${BACKEND} encoder=scott opt=${OPT_CONDITION} seed=${SEED}"
echo "Exp: ${EXP_NAME}"
