#!/bin/bash
#SBATCH --job-name=cnn-crate-muon-ha6
#SBATCH --output=slurm_logs/cnn_crate_muon_ha6_%A_%a.out
#SBATCH -N 1
#SBATCH --ntasks=1
...
# CNN encoder with CRATE heads and Optax Muon on actor/critic head matrices
# on humanoid and ant environments, 6 seeds.
#
# Fixed:
#   encoder_tanh_scale = 0.5
#   crate_step_size = 0.1
#   actor_muon_grad_norm = 100
#   critic_muon_grad_norm = 1
#
# 2 x 6 = 12 configs
...
GROUP_NAME="cnn_crate_muon_ha6_${GROUP_ID}"
export WANDB_RUN_GROUP="${GROUP_NAME}"
export WANDB_TAGS="cnn_crate_muon,encoder_cnn,opt_${OPT_CONDITION},env_${ENV_NAME},backend_${BACKEND},seed_${SEED},scale_${ENCODER_SCALE},crate,sweep6seeds,neurips_baselines"

EXP_NAME="ppo_cnn_crate_${OPT_CONDITION}_scale${ENCODER_SCALE}_${ENV_NAME}_b${BACKEND}_s${SEED}_t${TASK_ID}"
