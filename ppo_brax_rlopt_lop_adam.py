...
class LopReferenceCritic(nn.Module):
    """Reference loss-of-plasticity critic: two hidden layers on observations."""
    activation: str = "relu"
...
class LopReferenceActor(nn.Module):
    """Reference loss-of-plasticity actor: two hidden layers on observations."""
    action_dim: int
    activation: str = "relu"
    logstd_min: float = -5.0
    logstd_max: float = 2.0
...
if __name__ == "__main__":
    ...
    # Initialize networks
    if args.network_arch in {"lop", "lop_reference"}:
        if args.use_crate_network or args.use_crate_head:
            raise ValueError(
                f"--network-arch={args.network_arch} does not support CRATE "
                "trunk/head options."
            )
        network = IdentityNetwork()
        print(f"Using {args.network_arch} identity state trunk")
        ...
        raise ValueError(
            f"Unsupported --network-arch={args.network_arch!r}; "
            "expected 'ppo', 'lop', or 'lop_reference'."
        )
    actor_kwargs = dict(
    ...
    elif args.network_arch == "lop_reference":
        actor = LopReferenceActor(**actor_kwargs)
        critic = LopReferenceCritic(activation=args.actor_critic_activation)
        print("Using loss-of-plasticity reference two-layer actor/critic heads")
