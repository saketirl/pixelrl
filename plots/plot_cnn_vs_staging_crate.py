#!/usr/bin/env python3
"""Plot cached CNN Adam/Stiefel vs staging CRATE Adam/Stiefel curves."""
...
CURVES = [
    ...
    },
    {
        "source": "staging",
        "key": "crate_adam",
        "label": "CRATE Heads Adam",
        "color": "#CC79A7",
    },
    {
        "source": "staging",
        "key": "crate_stiefel",
        "label": "CRATE Heads Stiefel",
        "color": "#000000",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot CNN Adam/Stiefel vs staging CRATE Adam/Stiefel from cached CSVs."
    )
    ...
    parser.add_argument(
        "--plots-dir",
        type=Path,
        default=Path("plots/cnn_vs_staging_crate"),
    )
    parser.add_argument(
        "--envs",
        default=None,
...
def curve_frame(curve: dict, env: str, cnn: pd.DataFrame, staging: pd.DataFrame) -> pd.DataFrame:
    if curve["source"] == "cnn":
...
def plot_env(env: str, cnn: pd.DataFrame, staging: pd.DataFrame, output_path: Path) -> None:
